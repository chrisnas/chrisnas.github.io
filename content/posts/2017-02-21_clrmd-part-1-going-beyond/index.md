---
title: "ClrMD Part 1 - Going beyond SOS"
date: 2017-02-21T15:50:21.000Z
description: "A little bit of context Thousands of servers are closely monitored at Criteo and when…"
tags: ["C#", "ClrMD", ".NET"]
draft: false
cover:
  image: "clrInfo.png"
...
Lately, we got a few situations where the number of running threads was growing up to hundreds, even thousands. To limit the impact on production machines, we are taking dumps with [procdump](https://technet.microsoft.com/en-us/sysinternals/dd996900) from SysInternals and we are loading them into WinDBG to dig into the CLR data structures with SOS commands. However, when you are dealing with 20+ GB dumps and thousands of threads, the investigation starts to become complicated simply due to the mass of data, the performance hit and the complexity of the highly multi-threaded code.
...
One solution is to build tools that leverage sos commands textual output to automate well known scenari. The [LeakShell tool](https://codenasarre.wordpress.com/2011/05/18/leakshell-or-how-to-automatically-find-managed-leaks/) has been built as a WinDBG companion application to ease a memory leaks hunt. Even if LeakShell has been enhanced to [directly consume dumps](https://codenasarre.wordpress.com/2011/07/20/leakshell-1-4-use-the-dumps-luke/), parsing text outputs from !dumpheap -stat might be fragile if it changes but also not very scalable in large dumps.

As developers, we would definitively prefer to write our own tool based on clean and easy to use APIs instead of parsing cryptic textual output. This is exactly the purpose of ClrMD as stated by its first line of documentation: [CLR MD](https://github.com/microsoft/clrmd) *is a C# API used to build diagnostics tools*. This Microsoft project (thank you so much [Lee Culver](https://github.com/leculver)!) is available from [Github](https://github.com/Microsoft/clrmd) and provides a managed wrapper that brings the power of sos and symbol engine to your C# code. Take the time to [follow the tutorials](https://github.com/microsoft/clrmd/blob/main/doc/GettingStarted.md) and open the [samples code](https://github.com/microsoft/clrmd/tree/main/src/Samples): you have everything you need!
...
Note: this version 0.8.31 is still in beta but you could get the latest level of code from [Github](https://github.com/Microsoft/clrmd) (more on this in a later post).
...
Since .NET 4.0, it is possible to [load “several” versions of the CLR](https://msdn.microsoft.com/en-us/library/ee518876) in the same process at the same time. The list of the loaded CLR runtimes is provided by the DataTarget.ClrVersions property and for simplicity sake, only the first one will be taken into account in the rest of the series. This property returns a list of ClrInfo:

![](clrInfo.png)

Several members of this class relate to “Dac”: this makes reference to the data access layer provided by mscordacwks.dll. As most of you should know (if this is not the case, [read this detailed post](https://codenasarre.wordpress.com/2011/06/22/sending-an-sos/)), this library is used by sos.dll and ClrMD to access the internal CLR data structures. This sos/mscordacwks pair is unique per version of the CLR and therefore unique to the dumps you download from a server.
...
Clr = target.ClrVersions[0].CreateRuntime(
    Path.Combine(Path.GetDirectoryName(dumpFilename), "mscordacwks.dll")
    );
```
...
As explained in the [getting started page](https://github.com/microsoft/clrmd/blob/main/doc/GettingStarted.md), this call will try to use the “local” mscordacwks.dll from current machine Windows subfolder and if there is no matching version, it will use the DataTarget.SymbolLocator.Find method to download this dll from the Microsoft public servers.
...
The [syntax is simple](https://msdn.microsoft.com/en-us/library/windows/desktop/ee416588.aspx):
...
```text
SRV*http://msdl.microsoft.com/download/symbols;SRV*http://referencesource.microsoft.com/symbols
```
